...
class App extends Component {
  ...
  /**
   * Checks if there are up to date forecasts in the local storage.
   * If not it fetches new forecasts from the API
   * This is intended to be a quick solution for reduce the API requests.
   * @param cityId
   */
  checkForecast = (cityId) => {
    //If the key in the localStorage is not even set or if the current date
    // is after the last fetched prevision it fetches the forecasts from the API
    if (!localStorage.date || new Date().getTime() / 1000 > localStorage.date) {
      this.fetchForecasts(cityId);
    } else {
      //The data are already available from the localStorage
      this.setState(() => ({
        forecasts: JSON.parse(localStorage.forecasts)
      }))
    }
  };

  /**
   * Fetch the data from the API using api.openweathermap.org
   * secrets.apiKey is stored in secrets.json, this file is not committed
  ...
  fetchForecasts = cityId => {
    ...
    fetch(`${url}id=${cityId}&units=metric&APPID=${secrets.apiKey}`)
      .then(
        res => {
          //Check if the res is a positive one
          if (res.status === 200) {
            return res.json();
          }
        ...
        reason => {
          console.warn(`Promise has been rejected: ${reason}`)
        }
      )
      .then(res => {
        //Save the 1st forecast date for further comparison
        localStorage.date = res.list[0].dt;
  ...
  /**
   * Identify a city between the available ones.
  ...
  /**
   * Fetches the forecast for a new selected city
   */
  fetchNewCity = cityName => {
    //Get the city Id needed for the API
    const cityId = this.identifyCity(cityName);

    //Fetch the forecasts for the specific city
    this.fetchForecasts(cityId);
  };
    ...
    return (
      <div className="App">
        ...
        <City
          ...
          countryName={this.state.forecasts.city.country}
          forecastedDays={this.state.forecasts.list}
          fetchNewCity={this.fetchNewCity}
        />
      </div>
    );
  }
